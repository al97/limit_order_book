...
void TestIocCancelsRemainder() {
  ...
  assert(events[2].type == lob::EventType::Filled);
  assert(events[2].orderId == 1);
  assert(events[3].type == lob::EventType::Cancelled);
  assert(events[3].orderId == 2);
  assert(events[3].quantity == 7);
...
void TestIocOnEmptyBookCancels() {
  lob::ref::ReferenceBook book;
  const std::vector<lob::Event> events =
      book.Submit({1, lob::Side::Buy, 100, 10, lob::TimeInForce::IOC});

  assert(events.size() == 2);
  assert(events[0].type == lob::EventType::Accepted);
  assert(events[1].type == lob::EventType::Cancelled);
  assert(events[1].orderId == 1);
...
void TestIocOneSidedLeavesOppositeUnchanged() {
  lob::ref::ReferenceBook book;
  book.Submit({1, lob::Side::Buy, 100, 5, lob::TimeInForce::GTC});
  const std::vector<lob::Event> events =
      book.Submit({2, lob::Side::Buy, 101, 4, lob::TimeInForce::IOC});

  assert(events.size() == 2);
  assert(events[0].type == lob::EventType::Accepted);
  assert(events[1].type == lob::EventType::Cancelled);
  assert(book.GetRestingQuantity(1) == 5);
  assert(book.GetRestingQuantity(2) == 0);
...
void TestMarketIgnoresPriceAndNeverRests() {
  ...
  assert(events[2].type == lob::EventType::Trade);
  assert(events[2].orderId == 2);
  assert(events[2].price == 110);
  assert(events[2].quantity == 5);
  assert(events[3].type == lob::EventType::Filled);
  assert(events[3].orderId == 1);
  assert(events[4].type == lob::EventType::Filled);
  assert(events[4].orderId == 2);
  assert(events[5].type == lob::EventType::Cancelled);
  assert(events[5].orderId == 3);
  assert(events[5].quantity == 2);
...
void TestMarketOnEmptyBookCancels() {
  lob::ref::ReferenceBook book;
  const std::vector<lob::Event> events =
      book.Submit({1, lob::Side::Sell, 50, 8, lob::TimeInForce::Market});

  assert(events.size() == 2);
  assert(events[0].type == lob::EventType::Accepted);
  assert(events[1].type == lob::EventType::Cancelled);
  assert(events[1].quantity == 8);
  assert(book.Depth(lob::Side::Buy, 10).empty());
  assert(book.Depth(lob::Side::Sell, 10).empty());
  assert(book.GetRestingQuantity(1) == 0);
}
...
void TestFokFillsWhenLiquidityExists() {
  ...
  assert(events[4].type == lob::EventType::Filled);
  assert(events[4].orderId == 2);
  assert(events[5].type == lob::EventType::Filled);
  assert(events[5].orderId == 3);
...
void TestFokRejectsWithoutMutation() {
  ...
  for (const lob::Event& event : events) {
    assert(event.type != lob::EventType::Trade);
  }

  assert(book.GetRestingQuantity(1) == 3);
  assert(book.GetRestingQuantity(2) == 5);
  assert(book.GetRestingQuantity(3) == 0);
  assert(book.Top(lob::Side::Sell).price == 100);
  assert(book.Top(lob::Side::Sell).quantity == 3);
  assert(book.Depth(lob::Side::Buy, 10).empty());
}

void TestFokOverflowDoesNotWrapAvailability() {
  lob::ref::ReferenceBook book;
  const lob::Quantity huge =
      std::numeric_limits<lob::Quantity>::max() / 2 + 10;
  book.Submit({1, lob::Side::Sell, 100, huge, lob::TimeInForce::GTC});
  book.Submit({2, lob::Side::Sell, 101, huge, lob::TimeInForce::GTC});
  const std::vector<lob::Event> events =
      book.Submit({3, lob::Side::Buy, 101, 100, lob::TimeInForce::FOK});
  ...
  assert(events[2].type == lob::EventType::Filled);
  assert(events[2].orderId == 3);

  assert(book.GetRestingQuantity(1) == huge - 100);
  assert(book.GetRestingQuantity(2) == huge);
  assert(book.GetRestingQuantity(3) == 0);
}
...
int main() {
  ...
  TestIocOnEmptyBookCancels();
  TestIocOneSidedLeavesOppositeUnchanged();
  TestMarketIgnoresPriceAndNeverRests();
  TestMarketOnEmptyBookCancels();
